#!/usr/bin/env node
/**
 * unstable-plugin-smoke — verify the PACKED UNSTABLE extension surface end
 * to end with a real third-party-shaped plugin (Phase 3, plan §17):
 *
 *   1. the fixture imports ONLY the public `@xmoon76/dsh-pi-tui/
 *      extensions/unstable` (and the stable `extensions` entry for the
 ...
 *   4. the fixture exercises the UNSTABLE surface: UNSTABLE_API_LEVEL === 1,
 *      raw capture (observe/consume/rewrite), the low-level surface
 *      handle and the mount lease;
 ...
 *
 * Usage: node scripts/unstable-plugin-smoke.mjs [path-to-tgz]
 * @module unstable-plugin-smoke
 */
...
const FIXTURE_ROOT = join(PACKAGE_ROOT, 'test', 'fixtures', 'unstable-plugin')
...
function main() {
  ...
  const workDir = mkdtempSync(join(tmpdir(), 'unstable-plugin-'))
  try {
    ...
    check('packed package has dist/extension/unstable.d.mts', existsSync(join(pkgDir, 'dist', 'extension', 'unstable.d.mts')))

    // 2. IMPORT BAN (plan §17 CI gate): the fixture's IMPORT STATEMENTS
    ...
    // (extensions/unstable + extensions for the service type; cordis is a
    ...
    check('fixture imports ONLY the public extensions subpaths',
      dshImports.length === 2
        && dshImports.includes('@xmoon76/dsh-pi-tui/extensions/unstable')
        && dshImports.includes('@xmoon76/dsh-pi-tui/extensions'),
      dshImports.join(', '))

    // 3. UNSTABLE-SURFACE EXERCISE (plan §17 acceptance): the fixture
    //    exercises the Phase-3 unstable surface through the public entry.
    const surface = [
      ['UNSTABLE_API_LEVEL', 'tier level constant'],
      ['unstable(', 'facade builder'],
      ['ui.input.captureRaw', 'raw input capture'],
      ["mode: 'observe'", 'observe mode'],
      ["mode: 'capture'", 'capture mode'],
      ["action: 'consume'", 'consume decision'],
      ["action: 'rewrite'", 'rewrite decision'],
      ['ui.surface.handle', 'low-level surface handle'],
      ['mountComponent', 'low-level mount'],
    ]
    ...
    if (emit.status === 0) {
      writeFileSync(join(fixtureDir, 'runtime-probe.mjs'), `
        ...
        if (plugin.name !== 'dsh-pi-unstable-fixture') throw new Error('bad name: ' + plugin.name)
    ...
    if (process.env.UNSTABLE_SMOKE_KEEP !== '1') rmSync(workDir, { recursive: true, force: true })
  }

  for (const line of checks) console.log(line)
  if (failures.length > 0) {
    console.error(`\nunstable-plugin smoke FAILED (${failures.length}):`)
    for (const failure of failures) console.error(`  - ${failure}`)
    process.exit(1)
  }
  console.log('\nunstable-plugin smoke passed')
}
