...
- **A `subagent/descriptor` is no longer treated as a delegation Action.** It is the CHILD session's identity record (version / mode / provider / label), and a continuable child logs it BEFORE its first `turn/start` (upstream asserts `descriptorIndex < turnStartIndex`), so it belongs to no model turn: it never enters an Activity, the `N actions` stats or the `Action:` slot. The parent's own genuine `tool/call name=subagent` is the delegation evidence and carries the Action — the descriptor no longer synthesizes a second one.
- **A slash command always renders as its own standalone row, never folded into an Activity or an Action.** A command's lifecycle is session-level: DSH appends `command/run`/`command/done` as direct log-only events with **no model turn opened for them**, and renders the settled result outside model history (e.g. `✓ /compact`). The command card is therefore an INDEPENDENT transcript row: it is never an Activity member (it splits the surrounding Process run), never enters the `N actions` stats, and is never swallowed by the collapsed Focus `Action:` slot or Thought — so the common "agent finished, user runs `/compact` while idle" case keeps its feedback visible. The row's `turn` is only a legacy PLACEMENT artifact, not semantic ownership; explicit correlation via the official `sourceEventSeq` and a real `command` transcript kind remain follow-up work.
