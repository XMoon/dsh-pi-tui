...
- **Compact and Focus now share one collapsed `Think:` + `Action:` vocabulary, and both headers count `actions`.** The collapsed card no longer understands only genuine tool calls: the `Action:` slot shows the latest non-Thinking Process evidence in chronology — a genuine tool call, a live Preparing call, a subagent delegation, a slash command, an LLM retry, or an unpaired tool-result diagnostic (honestly rendered as `Unpaired … result`, never disguised as a successful call). The header follows with `N actions · subtype ×count` (e.g. `7 actions · read ×3 · bash ×2 · +1`, where `+1` counts the remaining action SUBTYPES): a genuine tool contributes its `callCount` (a merged read group is still two calls), a subagent / command (by its durable name, e.g. `/compact`) / retry occurrence contributes one each, an orphan result and Preparing contribute nothing, and `ask_user_question` / `exit_plan_mode` stay owned by their interaction panel — neither occupying the Action slot nor counting. Focus and Activity share one classifier, selection rule, formatter and subtype ordering; singleton stats such as `1 action · read ×1` are never special-cased away. Tool stays the strict underlying semantic: commands, retries and delegations do not become tools.
