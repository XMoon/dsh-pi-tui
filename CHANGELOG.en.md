...
- **Compatible with DeepSeek Harness `0.1.7-alpha.2`: settings move to profile-owned plugin configuration and agent presets switch to the official declarative registry.** TUI preferences (theme, footer, display preset, notifications, key bindings, …) now live in the `tui-app` plugin's profile configuration; `/settings` changes apply immediately and persist per field. Existing preferences in the legacy `settings.yaml` migrate automatically on first start (including the Focus-preference convergence into display presets and the input-history move); the migration only reads the legacy file, is retryable, and never overwrites values changed after migration. The agent preset roster is now the official `standard` / `ptc` / `minimal` / `cordis` declarations (default selection and "set as default" are adjudicated by the Host), and the guessed alias mapping for the legacy `code` default is gone. The minimum runtime is now `dsh 0.1.7-alpha.2`.
