...
- **Commands activate only from the USER-layer trusted source (definition trust + activation trust).** `ConfigPort.footerCustomItems.get()` or the validated result of a successful `/footer` save is the only executable definition source; executable ids = USER-trusted definitions ∩ USER-trusted active layout (`ConfigPort.footerCommandTrust.userFooterLayout`, or the validated layout of a `/footer` save). Project/merged configuration can neither supply a command string nor activate a dormant USER command item through a merged layout (unsaved drafts and failed saves never execute either).
