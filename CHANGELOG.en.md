...
- **Transcript disclosure, search and viewport now converge on one owner and reveal model.** Every hidden region has one operable owner on the current surface: the regular surface uses the shared `Ctrl+O` master for Work, Context clusters, ordinary folds and long/pending-user folds; when that key is unavailable (remapped away or disabled) those folds all fail open and no un-openable header or `(ctrl+o to expand)` hint is rendered. Expanded Focus now presents each contiguous Process run as an independently collapsible nested `Work` container (collapsed by default on fullscreen, openable by mouse or temporary search reveal; regular Focus keeps it fully revealed while its Thought is expanded), and an explicit Thought collapse returns that turn's nested Work to Compact depth. A mouse click on an internal blank row now collapses the NEAREST shared container (nested Work beats the outer Thought, a cluster-internal blank collapses the cluster, a boundary/global blank is inert). Search reveal is now a container path: navigation temporarily opens every hidden ancestor without writing manual state, an ordinary dismiss promotes those owners atomically, and an explicit collapse revokes the temporary reveal. Disclosure transitions still reuse the existing semantic viewport transactions (no second viewport model).
