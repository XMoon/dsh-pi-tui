...
- **Compact's Tool slot now shows PTC active sub-calls like Focus.** For example `Code · Bash ×2 running`; width degradation preserves the running state before sacrificing root description text, and nested child start/settle refreshes the collapsed Activity card immediately.
- **The `tools` stat semantics now agree between the Focus header and the Activity header.** `N tools` counts genuine model tool calls only; subagent delegations are counted separately as `N subagents` (no longer also counted as tools); synthetic rows such as slash commands no longer inflate the tool count — one label, one counting unit on both surfaces.
