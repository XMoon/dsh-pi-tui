...
- **Compact and Focus now share one collapsed `Think:` + `Action:` vocabulary, and both headers count `actions`.** The collapsed card no longer understands only genuine tool calls: the `Action:` slot shows the latest non-Thinking TURN-OWNED Process evidence in chronology — a genuine tool call, a live Preparing call, a subagent delegation, an LLM retry, or an unpaired tool-result diagnostic (honestly rendered as `Unpaired … result`, never disguised as a successful call). The header follows with `N actions · subtype ×count` (e.g. `7 actions · read ×3 · bash ×2 · +1`, where `+1` counts the remaining action SUBTYPES): a genuine tool contributes its `callCount` (a merged read group is still two calls), a subagent and a retry occurrence contribute one each, an orphan result and Preparing contribute nothing, and `ask_user_question` / `exit_plan_mode` stay owned by their interaction panel — neither occupying the Action slot nor counting. Focus and Activity share one classifier, selection rule, formatter and subtype ordering; singleton stats such as `1 action · read ×1` are never special-cased away. Tool stays the strict underlying semantic: retries and delegations do not become tools.
...
- **The Activity header now shows the process span's OWN wall-clock duration.** The timing is a presentation-only sidecar sourced entirely from session event times (thinking, tool call/result, command run/done, plus point evidence from retries and subagent delegations), aggregated inside the existing single summary walk; running cards tick with the existing repaint heartbeat (no per-card timers), missing evidence omits the duration (never a fake `0s`), a consecutive-read group that spans turns drops its timing so an Activity span never crosses a turn boundary, and the Preparing → durable tool-call handoff keeps the earliest authoritative start so elapsed seconds no longer reset from `3s` to `0s`. Token usage stays on the Focus header (a turn-level fact); Activity aggregates a Process span, has no trustworthy per-span usage attribution, and therefore NEVER displays, estimates or allocates tokens.
