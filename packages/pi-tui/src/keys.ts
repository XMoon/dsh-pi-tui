...
// Retain the parser's event-type state for the existing key parser.
let _lastEventType: KeyEventType = "press";

// The event-type suffix is meaningful only on a complete Kitty key
// sequence. Anchoring the classifier prevents ordinary text such as
// `hello:2u` from being mistaken for a protocol artifact.
const KITTY_EVENT_SEQUENCE = /^\x1b\[\d+(?::\d*)?(?::\d+)?;\d+:(?<event>[23])(?:u|~|[ABCDHF])$/u;

function kittyEventType(data: string): KeyEventType | undefined {
	const match = KITTY_EVENT_SEQUENCE.exec(data);
	if (match === null) return undefined;
	return match.groups?.event === "2" ? "repeat" : "release";
}

/**
 * Check if the input is a complete Kitty key release event.
...
/**
 * Check if the input is a complete Kitty key repeat event.
 * Only meaningful when Kitty keyboard protocol with flag 2 is active.
 */
