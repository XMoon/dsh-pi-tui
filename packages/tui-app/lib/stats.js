...
export function computeStats(events) {
        ...
        switch (event.type) {
            ...
            case 'request/context': {
                if (event.data.contextWindow !== undefined)
                    stats.contextWindow = event.data.contextWindow;
                break;
            }
            default:
                break;
    ...
    if (firstTokenCount > 0)
        stats.firstTokenMsAvg = firstTokenTotal / firstTokenCount;
    const streamMs = outputMs.reduce((sum, ms) => sum + ms, 0);
    if (streamMs > 0)
        stats.tokensPerSec = Math.round((stats.outputTokens * 1000) / streamMs);
    const totalInput = stats.inputTokens + stats.cacheReadTokens;
    if (totalInput > 0)
        stats.cacheHitPct = (stats.cacheReadTokens * 100) / totalInput;
    return stats;
}
 ...
 */
export function formatStats(stats, contextTokens) {
    const piParts = [
