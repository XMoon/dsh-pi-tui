...
- `/tasks` — 合并任务浏览器:后台 job 与子代理在同一个可搜索列表中
  ...
  按下 `Enter` 之前就能知道查看器是否可交互;durable 后代树以缩进行
  渲染(`├─` 按深度缩进),子代理的子代理无需进入父节点就能在这里
  看到。`Enter` 打开详情(子代理为子查看器——嵌套行只读——job 为
  状态查看器),`i` 中断选中的子代理。`/subagents` 是别名。
...
- `Ctrl+O` — 展开/折叠最近回合的可折叠条目(工具、system 卡片);
  ...
  "本地 shell")——但全屏 Focus 除外:那里 Ctrl+O 拥有 Thought root
  的整体开关:没有任何 Thought 展开时展开最近 3 个,有任意 Thought
  展开时全部收起(单个 secondary 细节仍由鼠标控制,Ctrl+O 不会
  full-reveal 它们)。Thinking 的 detail 有自己的拥有者——见 `Alt+T`。
...
- `@` — 编辑器中的文件/文件夹提及:`@` + Tab 从整个工作区补全文件
  ...
  - **子代理行**(durable 后代树,标签携带目录 mode:`continuable` 或
    `one-shot`,按深度缩进并带 `├─` 连接符——子代理的子代理在这里
    直接可见,顺序为稳定的 pre-order)——`Enter` 打开子代理查看器:
    **直接(深度 1)continuable** 子代理的查看器是**可交互的**——输入
    后续消息并回车,它会作为子代理的**下一回合**通过
    ...
    标签、activity、cwd 与子代理自己的 turns/steps/stats),退出后恢复
    主会话页脚。`i` 中断选中的子代理。它们从不注册 job 记录,所以此
    浏览器是它们唯一可一览的归宿。
...
`! command` 在本地运行命令,并把完成后的命令 + 输出作为普通用户消息
提交给会话;`!! command` 完全本地运行——卡片是唯一记录(绝不发给模型)。
卡片**默认折叠**,长日志不会塞满 TUI:运行中的卡片显示最新 5 行,
已结束的卡片最多显示 20 个视觉行(超长行会换行并按多行计),各自带
诚实的隐藏行数标记。`Ctrl+O`(与折叠最近工具回合的同一个主开关)把
卡片展开到 retained buffer——命令仍在跑时实时跟随——但全屏 Focus
除外:那里 Ctrl+O 拥有 Thought root 的整体开关,shell 卡片保持折叠;
`Alt+K` 快速清除
**已结束**的卡片(运行中的卡片绝不会被清除,进程不会被取消,已经
提交的 `!` context payload 不受影响)。
