...
test('Focus ON running: the Thought card is collapsed with previews, the process is hidden, the WorkingIndicator stays', async () => {
  ...
  assert.ok(joined.includes('Thinking: locating the transcript path…'), `narrative preview missing:\n${joined}`)
  ...
  assert.ok(!joined.includes('Read src/transcript.ts'), `collapsed must hide the tool card:\n${joined}`)
...
test('clicking the Thought header expands a RUNNING turn and reveals the live process', async () => {
  ...
  assert.ok(joined.includes('Read src/transcript.ts'), `expanded tool card must appear:\n${joined}`)
...
test('a collapsed running turn stays collapsed after turn/end (◐ → ▸) and shows only the final', async () => {
  ...
  assert.ok(!joined.includes('Read src/transcript.ts'), 'collapsed process stays hidden after turn/end (no tool card)')
...
test('Ctrl+O cannot leak a collapsed Focus turn (outer gate)', async () => {
  ...
  assert.ok(!joined.includes('Read src/transcript.ts'), `Ctrl+O must not leak collapsed tools:\n${joined}`)
