...
async function tempRoot(): Promise<string> {
  return mkdtemp(join(tmpdir(), 'dsh-attachment-'))
}

test('probeAttachment classifies generic files from metadata and a bounded prefix', async () => {
...
test('a supported raster signature stays on the image path', async () => {
  const root = await tempRoot()
  try {
...
test('generic admission streams bounded chunks and reuses recalled refs', async () => {
  const root = await tempRoot()
  try {
...
test('file mutation before or during streaming rejects and never completes admission', async () => {
  const root = await tempRoot()
  try {
...
test('a sparse generic file larger than the image resident cap is admitted as metadata', async () => {
  const root = await tempRoot()
  try {
