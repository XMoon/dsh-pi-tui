...
test('bounded process execution kills descendants after timeout', { timeout: 5_000 }, async (t) => {
  const life = testLifecycle(t)
  const directory = life.tempDir('dsh-process-test-')
...
test('test-environment preparation bounds its package-manager child tree', { timeout: 5_000 }, async (t) => {
  const life = testLifecycle(t)
  const directory = life.tempDir('dsh-prepare-process-test-')
...
test('SIGTERM lets test-environment preparation restore temporary metadata', { timeout: 10_000 }, async (t) => {
  const life = testLifecycle(t)
  const directory = life.tempDir('dsh-prepare-signal-test-')
  const executable = join(directory, 'fake-pnpm.mjs')
