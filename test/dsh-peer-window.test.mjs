...
test('all DSH runtime peers use the published npm 0.1.6-alpha.2 lower bound', () => {
  ...
  for (const [name, range] of dshPeerEntries) {
    ...
    assert.equal(semver.satisfies('0.1.6-alpha.2', range), true, `${name} must include the published npm floor`)
