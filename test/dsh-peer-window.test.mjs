...
const expectedWindow = '>=0.1.3-alpha.1'
...
test('all DSH runtime peers use the pinned master 0.1.3-alpha.1 lower bound', () => {
  ...
  for (const [name, range] of dshPeerEntries) {
    ...
    assert.equal(semver.satisfies('0.1.3-alpha.1', range), true, `${name} must include the pinned master floor`)
...
test('all DSH development packages stay pinned to the exact target rc', () => {
  ...
  for (const [name, version] of dshDevEntries) {
    assert.equal(version, '0.1.2-rc.1', `${name} must stay exact`)
  }
})
