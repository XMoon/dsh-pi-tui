...
const expectedWindow = '>=0.1.2-alpha.1 <0.1.3'
...
test('all DSH runtime peers use the validated 0.1.2 support window', () => {
  assert.ok(dshPeerEntries.length > 0, 'the bundle must declare DSH runtime peers')
  for (const [name, range] of dshPeerEntries) {
    assert.equal(range, expectedWindow, `${name} must use the validated support window`)
    assert.equal(semver.validRange(range) !== null, true, `${name} peer range must be valid semver syntax`)
    assert.equal(semver.satisfies('0.1.2-alpha.1', range), true, `${name} must include the target alpha`)
    ...
    assert.equal(semver.satisfies('0.1.3-alpha.1', range), false, `${name} must exclude the next prerelease line`)
    assert.equal(semver.satisfies('0.1.3', range), false, `${name} must exclude the next release line`)
  }
})
...
test('all DSH development packages stay pinned to the exact target alpha', () => {
  ...
  for (const [name, version] of dshDevEntries) {
    assert.equal(version, '0.1.2-alpha.1', `${name} must stay exact`)
  }
})
