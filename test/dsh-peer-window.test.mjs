...
const expectedWindow = '>=0.1.6-alpha.1'
...
test('all DSH runtime peers use the published npm 0.1.6-alpha.1 lower bound', () => {
  ...
  for (const [name, range] of dshPeerEntries) {
    ...
    assert.equal(semver.satisfies('0.1.6-alpha.1', range), true, `${name} must include the published npm floor`)
    ...
    assert.equal(semver.satisfies('0.1.6', range), true, `${name} must remain open to the matching stable release`)
