...
const expectedWindow = '>=0.1.3-alpha.2'
...
test('all DSH runtime peers use the published npm 0.1.3-alpha.2 lower bound', () => {
  ...
  for (const [name, range] of dshPeerEntries) {
    ...
    assert.equal(semver.satisfies('0.1.3-alpha.2', range), true, `${name} must include the published npm floor`)
    assert.equal(semver.satisfies('0.1.3-alpha.1', range), false, `${name} must reject the previous alpha line`)
    assert.equal(semver.satisfies('0.1.2-rc.1', range), false, `${name} must reject the previous rc line`)
    assert.equal(semver.satisfies('0.1.2', range), false, `${name} must reject the previous stable release`)
    assert.equal(semver.satisfies('0.1.3-alpha.0', range), false, `${name} must exclude the earlier alpha`)
    assert.equal(semver.satisfies('0.1.3', range), true, `${name} must remain open to later compatible releases`)
  }
})

test('all DSH development packages stay pinned to the exact target alpha.2', () => {
  ...
  for (const [name, version] of dshDevEntries) {
    assert.equal(version, '0.1.3-alpha.2', `${name} must stay exact`)
  }
})
