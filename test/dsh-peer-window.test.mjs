...
const expectedWindow = '>=0.1.2-rc.1'
...
test('all DSH runtime peers use the open-ended 0.1.2-rc.1 lower bound', () => {
  ...
  for (const [name, range] of dshPeerEntries) {
    ...
    assert.equal(semver.satisfies('0.1.2-rc.1', range), true, `${name} must include the target rc.1`)
    assert.equal(semver.satisfies('0.1.2', range), true, `${name} must include the target stable release`)
    assert.equal(semver.satisfies('0.1.2-alpha.4', range), false, `${name} must exclude the earlier alpha line`)
