...
test('runtime boundary accepts the recommended npm 0.1.7-rc.1 advisory notice', () => {
  const output = [
    `dsh-pi-tui v${bundleVersion} requires DeepSeek Harness 0.1.7-rc.1 or later,`,
...
test('runtime boundary rejects a stale candidate bundle-version label', () => {
  ...
  const output = [
    'dsh-pi-tui v0.0.1 requires DeepSeek Harness 0.1.7-rc.1 or later,',
    'but this installation is running dsh 0.1.1-rc.2.',
    'Upgrade DeepSeek Harness:',
    '  npm install -g --allow-scripts=@deepseek-ai/dsh-subprocess-local,koffi,node-pty,@google/genai,protobufjs,fs-ext @deepseek-ai/dsh@0.1.7-rc.1',
    'Then re-run: dsh --profile pi-tui',
