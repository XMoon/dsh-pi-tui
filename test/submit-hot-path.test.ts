...
function makeHarness(home: string, initial?: { id: string; events: SessionEvent[] }): {
  ...
  if (initial !== undefined) {
    liveSession = { id: initial.id, header: { id: initial.id, cwd: home, createdAt: 1_700_000_000_000, version: 1 }, events: initial.events }
  ...
  const agents = {
    create: async ({ sessionId }: { sessionId: string }) => {
      ...
      const session: LiveSession = { id: String(sessionId), header: { id: String(sessionId), cwd: home, createdAt: Date.now(), version: 1 }, events: [] }
