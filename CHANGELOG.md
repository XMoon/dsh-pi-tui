...
- **Compact 与 Focus 的折叠过程统一为 `Think:` + `Action:` 两个槽，头部统一按 `actions` 计数。** 折叠卡不再只认识「真实工具调用」：`Action:` 槽按时间顺序显示最新的非思考过程证据——真实工具调用、正在 Preparing 的调用、subagent 委派、斜杠命令、llm 重试，以及无法配对的工具结果诊断（诚实显示为 `Unpaired … result`，绝不伪装成成功调用）。头部随之统一为 `N actions · 子类型 ×次数`（如 `7 actions · read ×3 · bash ×2 · +1`；`+1` 计的是剩余**子类型种类**）：真实工具按其 `callCount` 计入（合并的 read 分组仍是 2 次调用），subagent / 命令（用其持久化名称，如 `/compact`）/ 每次重试各计一次，孤儿结果与 Preparing 不计入，`ask_user_question` / `exit_plan_mode` 仍由其交互面板独占、既不占据 Action 槽也不计入。Focus 与 Activity 共用同一套分类、选择、格式化与子类型排序规则；`1 action · read ×1` 这类单次计数不会被特殊隐藏。Tool 仍是严格的底层语义：命令、重试、委派都不会因此变成工具。
