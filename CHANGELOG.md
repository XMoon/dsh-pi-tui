...
- **`/resume <参数>` 共享同一生命周期。** 直达 id/prefix 匹配对着同一次
  input-first listing 解析（不再预同步 list，miss 时也不会二次 list）：overlay
  立即打开；唯一匹配时关闭 picker 并在 handler 返回前启动切换（保留旧同步语义，
  Esc 可取消）；未匹配则落入同一个 picker，参数保留为实时搜索词。
