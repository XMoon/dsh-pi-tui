...
- **长会话全文搜索显著更快。** 连续输入、`Enter`/`Shift+Enter` 跳转不再对每个字符
  重复重建整棵会话内容、重复测量或整窗重绘；同一窗口内的跳转近似即时，跨窗口跳转
  只有一次必要的重新投影。
...
- **搜索揭示是临时的。** `Esc` 关闭搜索后会恢复你原本的展开/折叠状态；搜索期间
  临时展开的 Thinking、工具/系统/压缩卡片、超长用户消息、嵌套 PTC 子调用以及被
  Workflow 大 phase 汇总隐藏的成员都能被定位到，但不会覆盖你自己手动做出的展开/
  折叠选择。关闭搜索后不会留下任何搜索造成的高亮或展开。
