...
| `surface-decisions.md` | contributors | Plain-`exit` quitting, background-subagent notices in the queue pane, the /login credential-target resolution, the Web-parity tool-card rendering rules (web cards, structured rawInput, plan content), and the mode-aware subagent viewer contract (continuable = interactive follow-up editor, one-shot = read-only; the single `ctx.subagents.followup` write path, per-child drafts, the child-owned footer while viewing) |
