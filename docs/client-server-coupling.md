...
| `src/runtime/presentation-read-port.ts` | (none) | Structural D1.3 presentation read contract; retains ordered mixed durable/transient inputs, window flags, and open state without a TUI or transport object. |
...
| `src/runtime/remote/presentation-read-remote.ts` | (none) | Experimental D1.3 adapter over official `SessionBinding.eventSource`, `SessionFace` snapshots, and `loadOlder()` only; it preserves source order and protects detached payloads. |
