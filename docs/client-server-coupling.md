...
| `src/runtime/direct/session-direct.ts` | `import:dsh-session`, `sessionPersistence`, `sessionQuery`, `sessionProjectionCache`, `agentPresets`, `tokenMeter` | The Direct `SessionReader` adapter (M1.3, extended M1.11 + alpha.2) — owns the lightweight live-preferred listing, the zero-I/O projection-cache-hinted preset enrichment (`sessionProjectionCache.cachedSnapshot` for cold rows, bounded `observeSession()` for cache misses only — the header snapshot captured by `list()` removes the second corpus listing), bounded content search, cached title batches, best-effort context measurement (`measureContext`, the /status row) and export read (`readExportData`); the consumer (commands.ts) depends on the port. The runtime `SessionId` import was added for the DSH 0.1.2 semantic query boundary. Baseline entries added by the M1.3/M1.11 relocations and the alpha.2 projection-cache hint. |
