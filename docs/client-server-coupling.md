...
| `src/runtime/direct/session-direct.ts` | `import:dsh-session`, `sessionPersistence`, `sessionQuery`, `sessionProjections`, `agentPresets`, `tokenMeter` | The Direct `SessionReader` adapter (M1.3, extended M1.11) — owns the lightweight live-preferred listing, cancellable bounded preset enrichment, bounded content search, cached title batches, best-effort context measurement (`measureContext`, the /status row) and export read (`readExportData`); the consumer (commands.ts) depends on the port. The runtime `SessionId` import was added for the DSH 0.1.2 semantic query boundary. Baseline entries added by the M1.3/M1.11 relocations. |
| `src/runtime/direct/session-preset-direct.ts` | `import:dsh-agent`, `import:dsh-session`, `sessionPersistence`, `agentPresets` | The Direct session-preset adapter — materializes cold sessions through DSH's official `agentPreset` projection before resume; legacy `code` → `ptc` translation is roster-aware and stays at the persisted identity seam in `src/runtime/session-preset.ts`. |
