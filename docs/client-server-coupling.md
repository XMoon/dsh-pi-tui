...
| `src/runtime/direct/session-writer-direct.ts` | `sessionTitle` | The Direct `SessionWriter` adapter (D2.1) — identity-based (sessionId) semantic operations over the live Agent resolver and the `ctx.sessionTitle` service: explicit-mode ordinary prompts, dsh-web-style FIFO per-occurrence Ctrl+S `steerQueued` calls, exact queue removal, user cancel with pending inbox preserved, normalized rename, and title refresh. Steer orchestration, barriers and revalidation stay in the runner. Baseline entry retained from the M1.4 relocation and updated for the converged contract. |
| `src/runtime/pending-input-reader-port.ts` | (none) | Semantic pending-input projection: running state plus `queued` / `steering` / `context` placements. It contains no Direct or Remote transport vocabulary. |
| `src/runtime/direct/pending-input-reader-direct.ts` | (none; structural Direct read) | The Direct `PendingInputReader` adapter — the only consumer-facing read adapter that knows `Agent.inbox.nextTurn` / `nextStep`; it maps them to semantic placements and returns `undefined` when the session is unavailable. |
...
| `src/runtime/session-lifecycle-port.ts` | (none) | The session LIFECYCLE port interface (D2.1 contract convergence) — transport-neutral semantic `create`/`open` operations and `SessionHandle` (no Host types). `OpenSessionRequest` is the semantic name even though its transitional identity field remains `resumeSessionId`; provider/model and fork seed metadata are still Direct-era inputs owned by D2.3/D2.4. Cancellation is client-local and never serialized. Zero Host coupling. |
| `src/runtime/direct/session-lifecycle-direct.ts` | `agents`, `import:dsh-agent`, `import:dsh-session` | The Direct `SessionLifecycle` adapter (D2.1 contract convergence) — the ONLY module converting semantic `create`/`open` requests into the Direct shapes (preset composition → `setup` callback, `SessionId`, seed) and mapping the client-local cancellation signal to `agents.create` / `agents.resume`; `agents.resume` is an implementation detail hidden behind semantic `open`. The runner keeps the process-local surface coordination (transition gate, operation barrier, generation/stale fences) around the port calls; DSH `SessionHandle` / `SessionWriteLease` is the cross-process writer authority. Baseline entries retained from the M1.5 relocation. |
