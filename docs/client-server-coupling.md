...
| `src/runtime/session-lifecycle-port.ts` | `import:dsh-agent`, `import:dsh-session` | The session LIFECYCLE port interface (M1.5) — type-only imports of the Host types the port contract needs. |
| `src/runtime/direct/session-lifecycle-direct.ts` | `agents`, `import:dsh-agent` | The Direct `SessionLifecycle` adapter (M1.5) — wraps the raw `agents.create` / `agents.resume`; the runner keeps the ownership machinery (lock/lease/PINNED/transition/barrier) around the port calls. Fork/rewind ride the existing session-fork.ts seams, wired through this port. Baseline entries added by the M1.5 relocation. |
