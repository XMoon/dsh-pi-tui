...
```sh
# 推荐：安装本 TUI 当前验证过的 exact DSH（0.1.2-alpha.3 属于 npm alpha
...
by the current Source baseline is `0.1.2-alpha.3`; the peer contract stays
...
follows npm's `latest` dist-tag, which is NOT the alpha.3 line — always name
