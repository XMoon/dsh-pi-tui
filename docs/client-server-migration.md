...
| M2 | Experimental Remote backend against an existing DSH Host (read-only first, then writes, then approval/question via `PendingWait` + `api.respond`) | Shadow parity on read paths; no session lock/lease in Remote mode |
