...
10. **Surface catalog: prefetch + coordinator, no startup probe (for now).** The first input must eventually see the effective agent-scoped command + human-skill catalog, but opening the TUI must not create a chat session: `--session` PREFETCHES the resumed agent's catalog before mount (synchronous install during command registration); the deferred start installs nothing and the first real session's coordinator refresh (awaited before the first submission dispatches) swaps in the live catalog. **Composition probes are DISABLED**: host-level `session/created` observers (dsh-permission-presets) write durable knob events into every fresh session, so any probe fails the zero-event gate and materializes a session artifact (200ms write-behind) — verified empirically. The probe module and the coordinator's composition target stay with their guards for a future Host probe/read-only mode. Post-mount refreshes go through ONE `CatalogRefreshCoordinator` (epoch + abort + latest-only commit; target changes turn old skill wrappers into revalidating transitions; a probed command missing from the real session is consumed with an explicit error, never sent to the model). Full contract: `docs/surface-catalog.md`.
...
- **Zero-event catalog probes.** IF a probe ever runs again, its session must hold `events.length === 0` before dispose or startup FAILS — never delete artifacts, skip flushes, or filter events to hide a violation. `CatalogProbeError.kind` separates retryable `create` failures from `post-create` violations that must propagate. Composition probes are currently DISABLED at every call site. Full contract: `docs/surface-catalog.md`.
