...
1. **Direct today, DSH-native client incrementally.** The production
   ...
   a TCP listener.
2. **Vendored fork, not npm dependency.** `@earendil-works/pi-tui` is vendored (pinned to v0.84.4 — see `packages/pi-tui/UPSTREAM.json`, the single source of truth). The re-vendor (2026-09) replaced the earlier kimi-code `44a6c70e` snapshot with the pristine Earendil baseline and re-applied the reviewed local divergences recorded as X001–X046; historical removed/absorbed records remain explicit in the ledger so an old deletion cannot silently return. `packages/pi-tui/vendor-divergences.json` is the authoritative schema-v2 audit ledger, and `packages/pi-tui/DIVERGENCES.md` is its generated human report — update the JSON and run `pnpm generate:pi-divergences`, never hand-edit the report. The per-frame processed-line reuse (X035) was restored after a benchmarked regression; its line cache compares primitive rendered values, while host render-array stability is a separate optimization. Re-verify every record on each re-vendor.
...
- **tmux `send-keys` batches keys — keep the two-step rhythm anyway**: `send-keys 'text' Enter` delivers the whole batch in a few ms. The re-vendored editor carries PasteBurst fallback X038 for terminals that lose bracketed-paste markers; drive the TUI with a pause: `send-keys 'text'`, sleep ≥0.3s, then `send-keys Enter` (full recipe: `docs/tmux-testing.md`).
...
- **Terminal queries (OSC 11 theme) must go through the ACTIVE screen**: in
  ...
  autodetect never lands. Same class as the editor render-routing fix
  (decision 8): route through the active screen and keep the reply listener
  registered on both.
